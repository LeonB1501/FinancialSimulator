...
export function runSimulationWithProgress(config, dslCode, initialCash, baseSeed, onProgress) {
    ...
    if (matchValue.tag === 0) {
        ...
        const configWithWarmup = new SimulationConfiguration(config.Assets, config.Correlations, config.TradingDays + warmupDays, config.Iterations, config.RiskFreeRate, config.Granularity, config.HistoricalData, config.StartDate, config.Scenario, config.ExecutionCosts, config.Tax);
        try {
